...
class TestAuthenticateAnonymousMode:
    ...
    def test_x_owner_and_valid_bearer_priority(self):
        ...
        # Bearer is processed first, returns 'default'
        assert result == "default"

    def test_x_owner_with_no_api_key_configured(self):
        """When no api_key is configured and Bearer is present, X-Owner is ignored."""
        request = create_mock_request(
            headers={
                "X-Owner": "alice",
...
class TestAuthenticateEdgeCases:
    ...
    def test_empty_bearer_token(self):
        """Empty Bearer token is treated as empty string, not matching api_key."""
