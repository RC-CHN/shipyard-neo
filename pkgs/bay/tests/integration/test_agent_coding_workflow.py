"""E2E-14: AI Agent Code Generation and Iterative Fix (Agentic Coding workflow) tests.
...
class TestE2E14AgentCodingWorkflow:
    """E2E-14: AI Agent Code Generation and Iterative Fix (Agentic Coding workflow)."""
...
"""
    ...
    async def test_agent_create_idempotency(self):
        """Agent should be able to safely retry sandbox creation with same idempotency key.
    ...
    async def test_agent_iterative_fix_with_multiple_files(self):
        ...
        Simulates:
        - Create main.py that imports from utils.py
        - utils.py has a bug
        - Agent fixes utils.py
        ...
        async with httpx.AsyncClient(base_url=BAY_BASE_URL, headers=AUTH_HEADERS) as client:
            ...
            try:
                # Write utils.py with bug (missing return)
...
"""
                ...
                # Execute - should "work" but add returns None
                exec1 = await client.post(
                    f"/v1/sandboxes/{sandbox_id}/python/exec",
                ...
                assert exec1.status_code == 200
                result1 = exec1.json()
                # Note: This will actually run but produce wrong output (None)
                # because missing return is not a syntax error
                assert result1["success"] is True
                assert "None" in result1["output"], \
                    "Buggy add() should return None"

                # Agent detects wrong output and fixes utils.py
...
"""
                ...
                # Need to reload modules - use a fresh namespace
                exec2 = await client.post(
                    f"/v1/sandboxes/{sandbox_id}/python/exec",
                    json={
                        "code": """
...
""",
    ...
    async def test_agent_variable_persistence_within_session(self):
        ...
        This is critical for agent workflows where:
        - Agent defines helper functions
        - Agent uses them in later steps
        - Agent accumulates state (e.g., data processing results)
        """
        async with httpx.AsyncClient(base_url=BAY_BASE_URL, headers=AUTH_HEADERS) as client:
            create_resp = await client.post(
                "/v1/sandboxes",
            ...
            try:
                # Round 1: Define helper function
                exec1 = await client.post(
                    f"/v1/sandboxes/{sandbox_id}/python/exec",
                    json={
                        "code": """
...
""",
                ...
                # Round 2: Use helper function and accumulate results
                exec2 = await client.post(
                    f"/v1/sandboxes/{sandbox_id}/python/exec",
                    json={
                        "code": """
